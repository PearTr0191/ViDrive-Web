...
import { useLocalePath, useCurrentLocale, stripLocale } from '../lib/seo'
...
function switchLocalePath(currentPath: string, targetLocale: Locale): string {
  ...
  const dest = stripped === '/' ? '' : stripped
  return `/${targetLocale}${dest}`
}
    ...
    <div className="min-h-screen relative" style={{ backgroundColor: 'var(--bg-base)', color: 'var(--text-primary)' }}>
      ...
      >
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
           <Link to={useLocalePath('/')} className="flex items-center gap-2.5 group" aria-label="ViDrive">
            <Logo height={28} />
          </Link>
           ...
           <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 p-1 rounded-lg border" style={{ borderColor: 'var(--border-default)', backgroundColor: 'rgba(0,0,0,0.2)' }} role="group" aria-label={t('a11y.language')}>
              <button
                onClick={() => {
                  setLocale('en')
                  navigate(switchLocalePath(location.pathname, 'en'), { replace: true })
                }}
                className={`px-2.5 py-1 rounded-md text-xs font-mono font-semibold transition-all ${
                  currentLocale === 'en' ? 'bg-accent text-[var(--bg-base)] border border-accent' : 'hover:opacity-90'
                }`}
                style={currentLocale === 'en' ? { color: 'var(--bg-base)' } : { color: 'var(--text-secondary)' }}
              ...
              <button
                onClick={() => {
                  setLocale('vi')
                  navigate(switchLocalePath(location.pathname, 'vi'), { replace: true })
                }}
                className={`px-2.5 py-1 rounded-md text-xs font-mono font-semibold transition-all ${
                  currentLocale === 'vi' ? 'bg-accent text-[var(--bg-base)] border border-accent' : 'hover:opacity-90'
                }`}
                style={currentLocale === 'vi' ? { color: 'var(--bg-base)' } : { color: 'var(--text-secondary)' }}
      ...
      {/* Main content.
          ...
          hero back-layer has no animated ancestor and the light image is genuinely static
